...
class SpecificModal extends Component {
  constructor (props) {
    super(props);
    let type = null;
    let isEdit = false;
    ...
    this.props.form.validateFieldsAndScroll((err, fields) => {
      if (!err) {
        ...
        if (this.state.isEdit) {
          ...
          url = `${config.host}/sys/role/update`;
        } else {
          url = `${config.host}/sys/role/save`;
        }
        ...
        <Form>
          <div>
            ...
            >
              ...
              })(<Input placeholder="请填写编码" disabled={isEdit} />)}
            </FormItem>
            <FormItem
              style={{ display: "flex", justifyContent: "center" }}
              {...formItemLayout}
...
export default Form.create()(SpecificModal);
