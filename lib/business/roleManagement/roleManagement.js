...
import { Row, Col, Form, Input, Icon, Divider, Button, Popconfirm, Modal, notification } from "antd";
...
import SpecificModal from "./roleCU/roleCU";
...
const options = {
  ...
  del (record, target) {
    request.POST(`${config.host}/sys/role/delete?id=${record.id}`).then((res) => {
      if (res.success) {
    ...
    return (
      <div style={style}>
        <FixedTool style={pageHeaderStyle}>
          <span style={{ marginLeft: 20 }}>角色管理</span>
        ...
        >
          {
            this.state.modalVisiable && <SpecificModal
